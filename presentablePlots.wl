...
modListPlot[data__,opts:OptionsPattern[{PlotMarkers->{"OpenMarkers",Thick,Medium},AxesFontSize->defaultAxesFontSize,GridLines->Automatic,ListPlot}]]:=
ListPlot[data,PlotRange->Full,Axes->None,Frame->{True,True,False,False},FrameStyle->Directive[Thick,Darker[Gray],FontSize->OptionValue[AxesFontSize]],FrameLabel->OptionValue[AxesLabel],PlotStyle->Thick,GridLines->OptionValue[GridLines](* to use the default from the OptionsPattern, the options must be explicitly used here, otherwise the default from ListPlot[] is used *),(* this trick ensures that the remainder of options is passed to ListPlot[], but restricted to the options known to ListPlot[] *)Evaluate@(Sequence@@FilterRules[{opts},Options[ListPlot]])]
modPlot[args__,opts:OptionsPattern[{AxesFontSize->defaultAxesFontSize,GridLines->Automatic,Plot}]]:=Plot[args,PlotRange->Full,Axes->None,Frame->{True,True,False,False},FrameStyle->Directive[Thick,Darker[Gray],FontSize->OptionValue[AxesFontSize]],FrameLabel->OptionValue[AxesLabel],PlotStyle->Thick,GridLines->OptionValue[GridLines](* to use the default from the OptionsPattern, the options must be explicitly used here, otherwise the default from Plot[] is used *),(* this trick ensures that the remainder of options is passed to Plot[], but restricted to the options known to Plot[] *)Evaluate@(Sequence@@FilterRules[{opts},Options[Plot]])]
