...
exportPlot[plot_,opts:OptionsPattern[{ImageResolution->600,fileIdentifier->"",outputDirectory->NotebookDirectory[],Export}]]:=Export[FileNameJoin[{OptionValue[outputDirectory],FileBaseName[NotebookFileName[]]<>"_"<>If[StringMatchQ[OptionValue[fileIdentifier],""],ToString[Hash[plot]],ToString[OptionValue[fileIdentifier]]]<>".png"}],plot,ImageResolution->OptionValue[ImageResolution],(* this trick ensures that the remainder of options is passed to Export[], but restricted to the options known to Export[] *)Evaluate@(Sequence@@FilterRules[{opts},Options[Export]])]
