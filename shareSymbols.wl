...
saveListToCSVFile::usage="saveListToCSVFile[fileBaseName,list] saves a list to a data file fileBaseName.csv in the default data directory of the current project;  useAsRelativePath->False uses the full path of <fileBaseName> rather than the data directory";
...
saveListToCSVFile[fileBaseName_String,list_,headers_:{},OptionsPattern[{useAsRelativePath->True}]]:=Export[FileNameJoin[{If[OptionValue[useAsRelativePath],setupEnvironments`projectDataDirectory[],Nothing],StringDelete[fileBaseName,".csv"(* avoid duplicate csv ending if function is called with ending *)]<>".csv"}],list,"TableHeadings"->If[headers==={},None,ExportString[ToString[#],"String"(* this proved very important for exporting Greek letters as symbols, so change with care *)]&/@headers],"TextDelimiters"->""];
