...
saveSymbolsToFile::usage="saveSymbolsToFile[fileBaseName, symbols, headers] saves a symbol or list of symbols to a file fileBaseName.mx in the default data directory of the current project using the optional column headers <headers>";
...
saveListToFile[fileBaseName_String,list_,headers_:{}]:=Export[FileNameJoin[{setupEnvironments`projectDataDirectory[],fileBaseName<>".csv"}],list,"TableHeadings"->If[headers==={},None,ExportString[ToString[#],"String"(* this proved very important for exporting Greek letters as symbols, so change with care *)]&/@headers]];
loadListFromFile[fileBaseName_String]:=Import[FileNameJoin[{setupEnvironments`projectDataDirectory[],fileBaseName<>".csv"}](* omitting "Table" or "Data" here proved very important for exporting Greek letters as symbols, so change with care *)];
